...
class Daily_tg:
    ...
    def tg():
        ...
        while True:
            ...
            todo_times = text_ocr.surplus_times('TG')
            print(f'TG任务还剩{todo_times}次'), time.sleep(0.1)
            press_button('alt', 'y')
            if todo_times == 0 :
                print('TG任务已经完成，无法继续')
                return 1997
            press_button('alt', 'v')
            time.sleep(2)
            text_ocr.ocr_target(region=(204, 180, 353-204,539-180), text='天罡', click_or_not=True)
            time.sleep(2)
